...
export default function TKAQuestionFields({ item, options, setOptions }: Props) {
  ...
  return <>
    <label>Bentuk soal TKA</label><select name="question_type" value={questionType} onChange={(event) => setQuestionType(event.target.value as QuestionType)}><option value="single_choice">Pilihan Ganda Sederhana</option><option value="multiple_choice">PGK MCMA (jawaban jamak)</option><option value="category">PGK Kategori</option></select>
    <input type="hidden" name="presentation_type" value="single"/><input type="hidden" name="group_code" value=""/><input type="hidden" name="stimulus_text" value=""/>
    <label>{questionType === "category" ? "Pertanyaan/petunjuk klasifikasi" : "Pertanyaan"}</label><textarea name="content_text" defaultValue={`${legacyStimulus}${item?.content_text ?? ""}`} required/>
    <ImageField label="Gambar pertanyaan (opsional)" value={questionImage} onChange={setQuestionImage}/>
    {questionType === "category" && <><label>Kategori jawaban</label><div className="option-rows">{categoryLabels.map((label, index) => <div className="option-row" key={index}><input value={label} onChange={(event) => setCategoryLabels((current) => current.map((value, itemIndex) => itemIndex === index ? event.target.value : value))} placeholder="Contoh: Benar" required/><button type="button" className="danger-action" disabled={categoryLabels.length <= 2} onClick={() => setCategoryLabels((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Hapus</button></div>)}</div><button type="button" className="table-action" onClick={() => setCategoryLabels((current) => [...current, ""])}>+ Tambah kategori</button></>}
    <label>{questionType === "category" ? "Daftar pernyataan" : "Pilihan jawaban"}</label>
    ...
    <button type="button" className="table-action" onClick={addOption}>+ Tambah {questionType === "category" ? "pernyataan" : "opsi"}</button>
