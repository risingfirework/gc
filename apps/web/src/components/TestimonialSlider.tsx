...
export default function TestimonialSlider({ platformName = "kami" }: { platformName?: string }) {
  ...
  return <section className="section" id="testimoni">
    <div className="section-heading"><div><p className="eyebrow">Testimoni</p><h2>Kata mereka tentang {platformName}</h2></div><SliderArrows onPrev={() => scroll(-1)} onNext={() => scroll(1)}/></div>
    <div className="slider-track" ref={trackRef}>
      {items.map((item) => <article className="card testimonial-card slider-item" key={item.id}>
        <p className="testimonial-quote">&ldquo;{item.quote}&rdquo;</p>
        <div className="testimonial-author"><span className="profile-avatar">{(item.user_name || item.user_email || "T")[0].toUpperCase()}</span><div><strong>{item.user_name || item.user_email}</strong>{platformName !== "kami" && <small>Siswa {platformName}</small>}</div></div>
      </article>)}
    </div>
  </section>;
}