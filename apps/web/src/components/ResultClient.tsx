...
import { AnswerReview, api, ExamResult, SiteSettings } from "@/services/api";
...
export default function ResultClient({ attemptID }: { attemptID: string }) {
  ...
  return <main><div className="container result-page"><nav className="dashboard-nav"><Link className="brand" href="/dashboard"><BrandLogo logoDataURL={settings?.logo_data_url}/>{settings?.platform_name??""}</Link><span className="nav-actions"><NotificationBell/><StudentMenu active="belajar" loggingOut={loggingOut} onLogout={() => void logout()} /></span></nav>
...
function QuestionReviewCard({ item, index, total }: { item: AnswerReview; index: number; total: number }) {
  ...
  const statusIcon = isEssay && !unanswered ? "…" : status === "correct" ? "✓" : status === "wrong" ? "×" : "–";
  const statusLabel = isEssay && !unanswered ? "Belum dinilai" : status === "correct" ? "Benar" : status === "wrong" ? "Salah" : "Tidak dijawab";
  return <article className={`card review-question-card ${status}`}>
    <header className="review-question-header"><div className="question-meta"><span>{item.subject_name}</span><span>Soal {index + 1} dari {total}</span></div><span className={`review-status ${status}`}><i aria-hidden="true">{statusIcon}</i>{statusLabel}</span></header>
    <div className="review-question-main">
      ...
      {item.question_type === "essay" && <p className="answer-instruction">Soal esai · dinilai manual oleh guru.</p>}
      {item.question_type === "essay" ? <div className="review-essay"><div className="essay-block student"><small>Jawaban Anda</small><p>{item.selected_option || "—"}</p></div><div className="essay-block reference"><small>Referensi jawaban</small><p>{item.correct_answer}</p></div></div> : item.question_type !== "category" ? <div className="review-answer-list">{item.options.map((option) => {
    ...
    <div className="review-analysis">
      <div className="review-analysis-facts"><p className="review-analysis-title">Hasil jawaban</p><dl><div><dt>Hasil</dt><dd className={status}>{status === "correct" ? "Jawaban benar" : isEssay && !unanswered ? "Menunggu penilaian manual" : status === "wrong" ? "Jawaban belum tepat" : "Belum dijawab"}</dd></div><div><dt>Jawaban Anda</dt><dd>{formatAnswer(item.selected_option, isEssay)}</dd></div><div><dt>Kunci jawaban</dt><dd>{formatAnswer(item.correct_answer, isEssay)}</dd></div><div><dt>Bobot soal</dt><dd>{item.score_weight}</dd></div></dl></div>
      <div className="review-explanation"><p className="review-analysis-title">Pembahasan</p><RichText content={item.explanation_text || "Pembahasan belum tersedia."}/>{item.explanation_video_url&&<a className="button secondary" href={item.explanation_video_url} target="_blank" rel="noreferrer">Tonton video pembahasan</a>}</div>
    </div>
  </article>;
}
...
function formatAnswer(value?: string, raw = false) {
  if (!value) return "—";
  if (raw) return value;
